...
- **交付**：完成 = 验证过。没跑的检查明说；不做"看不见的改动"；局限如实标注。**每笔功能改动收尾必须本地跑 `bash scripts/build.sh assembleDebug` 产出 APK 并向用户报告路径与装完后的版本号显示值**（版本号必须可区分新旧包——L026；APK 是用户的验收物；CI artifact 只是备份，不能替代本地出包）；**每批交付报告必须写明「这批用户会看到什么变化 / 看不到什么」**（分批重构时尤其如此——L027）；**同步用户可见信源：docs（feature_list/CHANGELOG/progress）+ App 内文案**——首页进度卡/说明文字里的里程碑口径过时 = 没交付（L014）
...
7. **对抗审查门禁（L021）**：每批 bug 修复 commit 前必派对抗审查代理（攻击面=本批 diff+边界+并发，只报有具体触发路径的真问题），P1 清零才发版；修复者对自己的修复有锚定盲区，已两次实证
