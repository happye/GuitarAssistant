...
- **交付**：完成 = 验证过。没跑的检查明说；不做"看不见的改动"；局限如实标注。**每笔功能改动收尾必须本地跑 `bash scripts/build.sh assembleDebug` 产出 APK 并向用户报告路径**（APK 是用户的验收物；CI artifact 只是备份，不能替代本地出包）；**同步用户可见信源：docs（feature_list/CHANGELOG/progress）+ App 内文案**——首页进度卡/说明文字里的里程碑口径过时 = 没交付（L014）
